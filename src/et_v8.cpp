...
// Title: TSort
// Version: v8
// Details: (DBMS) Batching (double) + Multi-threading + sampling
// Author: Zheng Ying
// Comment: 
//
////////////////////////////////////////////
...
void et_v8(uint32_t num, uint32_t treeNum, uint32_t batchSize, uint32_t threadNum1, 
    ...
    Tstart(startTimer);
    uint32_t bufferSize = (batchSize * 2 * treeNum - treeNum / 4) / 2;    
    ...
    for (uint32_t runIdx = 0; runIdx < runNum; ++runIdx) {
        ...
        // Run Insertion
        insertBufferWait = std::async(std::launch::async, [=, &pool, &treeUpp, &insertBuffer, &pmemAddr]() {
            vector<pair<pair<uint32_t, uint32_t>, uint32_t>> insertRange;     // [start, end]
            uint32_t start = 0;
            uint32_t preTreeIdx = findTree(insertBuffer->key, treeUpp, 0, treeNum-1);
            uint32_t curTreeIdx = 0;
            for (uint32_t i = 1; i < bufferSize; ++i) {
                curTreeIdx = findTree((insertBuffer + i)->key, treeUpp, 0, treeNum - 1);
