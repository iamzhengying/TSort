...
// Title: TSort
// Version: v1
// Details: (BHMS) Batching + Multi-threading + hashing
// Author: Zheng Ying
// Comment: 
//
////////////////////////////////////////////
...
void et_v1(uint32_t num, uint32_t treeNum, uint32_t batchSize, uint32_t threadNum1, 
    ...
    Tstart(startTimer);
    uint32_t bufferSize = batchSize * 2 * treeNum - (bucketNum + treeNumActual) / 4;  
    ...
    for (uint32_t runIdx = 0; runIdx < runNum; ++runIdx) {
        ...
        uint32_t start = 0;
        uint32_t preTreeIdx = bucketTreeMap[hash_i(readBuffer->key, bucketInterval)];
        uint32_t curTreeIdx = 0;
        for (uint32_t i = 1; i < bufferSize; ++i) {
            curTreeIdx = bucketTreeMap[hash_i((readBuffer + i)->key, bucketInterval)];
        ...
        for (uint32_t thrd = 0; thrd < threadNum5; ++thrd) {
            insertWait.emplace_back(
                pool.enqueue([=, &pmemAddr, &insertRange, &readBuffer, &treeSize]() {
