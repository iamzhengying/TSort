                              ...
                              size_t read_length_blk)
{
    ...
    size_t thrd_recs = read_length_blk * conf.idx_recs_per_blk / conf.read_thrds;
    ...
    for (int thrd = 0; thrd < conf.read_thrds; thrd++)
    {
        results.emplace_back(
            pool.enqueue([=, &read_buffer, &run_file_map]()->size_t
            {
                ...
                size_t thrd_file_off = file_off * conf.block_size + thrd_recs * conf.idx_record_size * thrd;
                size_t thrd_read_buf_rec_off = read_buf_rec_off + thrd_recs * thrd;
#ifdef bandwidth
                threadargs[thrd].tid = thrd;
                threadargs[thrd].start_time = rdtsc();
#endif
                while (read_recs < thrd_recs)
                {
                    pending_recs_sz = (thrd_recs * conf.idx_record_size - read_recs * conf.idx_record_size);
