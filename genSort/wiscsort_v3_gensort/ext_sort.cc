...
void ExtSort::MergeRuns(vector<string> runs, string input_file)
{
    ...
    for (uint32_t file_num = 0; file_num < runs.size(); ++file_num)
    {
        ...
        // 此处读取的recs必须是整的blk，但是blk的数量可以不被read_thrd均分（这里我改了MergeRead的代码
        size_t tmp = data_manager.MergeRead(read_buffer, cur_rec_off[file_num],
...
void ExtSort::Sort(vector<string> &files)
{
	...
	// for (int i = 0; i < KEY_SIZE; ++i) std::cout << output->k.key[i];
	// std::cout << "', last element is '";
	// for (int i = 0; i < KEY_SIZE; ++i) std::cout << (output + st.st_size / conf.record_size - 1)->k.key[i];
	// std::cout << "'." << std::endl;
