...
int32_t partition_median(RecordPtr* array, int32_t start, int32_t end);
void parallelQuickSort(RecordPtr* recordPtrs, int32_t start, int32_t end);
...
void quick(uint32_t num, uint32_t batchSize, uint32_t threadNum1, uint32_t threadNum2) {
    ...
    #pragma omp parallel num_threads(threadNum1)
    {
        ...
        parallelQuickSort(recordPtrs, 0, num-1);
    }
    Tend(endTimer);
    printf("Sorting ");
    singleLatency();
...
void parallelQuickSort(RecordPtr* recordPtrs, int32_t start, int32_t end) {
    if (start < end) {
        int32_t pivot = partition_median(recordPtrs, start, end);
        #pragma omp task default(none) firstprivate(recordPtrs, start, pivot)
        {
            parallelQuickSort(recordPtrs, start, pivot-1);
        }
        #pragma omp task default(none) firstprivate(recordPtrs, pivot, end)
        {
            parallelQuickSort(recordPtrs, pivot+1, end);
        }
    }
}
...
uint32_t choosePivotMedianOfThree(RecordPtr* array, uint32_t l, uint32_t r) {    
    uint32_t mid = (l + r) >> 1;
    if (((array[l].key - array[mid].key) * (array[l].key - array[r].key)) <= 0) {
        return l;
    } else if (((array[mid].key - array[l].key) * (array[mid].key - array[r].key)) <= 0)    {
        return mid;
    } else {
        return r;
    }
