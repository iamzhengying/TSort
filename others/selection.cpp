...
void selectionSortHelper(RecordPtr* recordPtrs, uint32_t num);
...
void selection(uint32_t num, uint32_t batchSize, uint32_t threadNum1, uint32_t threadNum2) {
    ...
    RecordPtr* recordPtrs = mmap_pmem_file<RecordPtr>(FILENAME_SELECT_PTR, num);
    Record* records_sorted = mmap_pmem_file<Record>(FILENAME_SELECT_SORTED, num);
    ...
    // selection sort
    selectionSortHelper(recordPtrs, num);
    Tend(endTimer);
    printf("Sorting ");
    singleLatency();
...
void selectionSortHelper(RecordPtr* recordPtrs, uint32_t num) {
    for (uint32_t i = 0; i < num; ++i) {
        ...
        for (uint32_t j = i+1; j < num; ++j) {
            if (recordPtrs[j].key < recordPtrs[minIdx].key) {
                minIdx = j;
            }
        }
        if (minIdx != i) {
            RecordPtr tmp;
            tmp.key = recordPtrs[minIdx].key;
            tmp.ptr = recordPtrs[minIdx].ptr;
            recordPtrs[minIdx].key = recordPtrs[i].key;
            recordPtrs[minIdx].ptr = recordPtrs[i].ptr;
            recordPtrs[i].key = tmp.key;
            recordPtrs[i].ptr = tmp.ptr;
        }
    }
}
