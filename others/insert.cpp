...
void insertion(uint32_t num, uint32_t batchSize, uint32_t threadNum1, uint32_t threadNum2) {
    ...
    RecordPtr* recordPtrs = mmap_pmem_file<RecordPtr>(FILENAME_INSERT_PTR, num);
    Record* records_sorted = mmap_pmem_file<Record>(FILENAME_INSERT_SORTED, num);
    ...
    for (uint32_t i = 1; i < num; i++) {
        memcpy(&kp, recordPtrs + i, sizeof(RecordPtr));
        uint32_t j = i - 1;
        if (recordPtrs[j].key <= kp.key) continue;
        while (recordPtrs[j].key > kp.key) {
            memcpy(recordPtrs + j + 1, recordPtrs + j, sizeof(RecordPtr));
            if (j > 0) j = j - 1;
            else break;
        }
        memcpy(recordPtrs + j, &kp, sizeof(RecordPtr));
    }
    Tend(endTimer);
    printf("Sorting ");
    singleLatency();
